...
namespace Securrency.TDS.WebClient
{
    public class TdsClient : ITdsClient
    {
        ...
        public async Task UploadWallets(WalletPostModel[] wallets, CancellationToken ct)
        {
        ...
        private async Task<T> DeserializeResponseAsync<T>(HttpResponseMessage message, CancellationToken ct)
        {
            if (message.IsSuccessStatusCode)
            {
                _logger.LogDebug("Successful response");
                using Stream stream = await message.Content.ReadAsStreamAsync();
                var result = await JsonSerializer.DeserializeAsync<T>(stream, null, ct);
                return result;
            }
